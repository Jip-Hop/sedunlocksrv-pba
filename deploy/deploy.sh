...
build_pba() {
    ...
    # build.sh always creates sedunlocksrv-pba-latest.img as a symlink to the
    # real timestamped image. Resolve it with realpath so we validate the actual
    # file — stat/dd on the symlink itself return symlink metadata, not file data.
    local latest_img
    local latest_link="${REPO_ROOT}/sedunlocksrv-pba-latest.img"
    if [ -L "${latest_link}" ]; then
        latest_img=$(realpath "${latest_link}" 2>/dev/null)
    else
        latest_img=$(ls -t "${REPO_ROOT}"/sedunlocksrv-pba-*.img 2>/dev/null | grep -v 'latest' | head -1)
    fi
